...
class Admin extends CI_Controller
{
    ...
    // ==========================================
    // PYP TEST SERIES & QUESTION MANAGEMENT
    // ==========================================

    public function pyp_papers($param1 = "", $param2 = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        if ($param1 == 'add') {
            ...
            if (is_array($sections_raw)) {
                $order = 1;
                foreach ($sections_raw as $s_name) {
                    if (!empty(trim($s_name))) {
                        $sections[] = array(
                            'id' => 'sec_' . strtolower(preg_replace('/[^a-zA-Z0-9]+/', '_', trim($s_name))),
                            'name' => trim($s_name),
                            'order' => $order++
                        );
                    }
            ...
            if (empty($sections)) {
                $sections = array(
                    array('id' => 'sec_reasoning', 'name' => 'General Intelligence & Reasoning', 'order' => 1),
                    array('id' => 'sec_quant', 'name' => 'Quantitative Aptitude', 'order' => 2),
                    array('id' => 'sec_ga', 'name' => 'General Awareness', 'order' => 3),
                    array('id' => 'sec_english', 'name' => 'English Comprehension', 'order' => 4)
                );
            }

            $slug = slugify($this->input->post('title'));
            if ($this->db->get_where('pyp_papers', array('slug' => $slug))->num_rows() > 0) {
                $slug .= '-' . time();
            }
            ...
            $data = array(
                ...
                'slug' => $slug,
                'paper_type' => $this->input->post('paper_type') ?: 'PREVIOUS_YEAR',
                'year' => (int)$this->input->post('year') ?: (int)date('Y'),
                'shift' => html_escape($this->input->post('shift')) ?: 'Shift 1',
                'duration_minutes' => (int)$this->input->post('duration_minutes') ?: 60,
                'total_marks' => (float)$this->input->post('total_marks') ?: 200.00,
                'positive_marks' => (float)$this->input->post('positive_marks') ?: 2.00,
                'negative_marks' => (float)$this->input->post('negative_marks') ?: 0.50,
                'is_free' => $this->input->post('is_free') ? 1 : 0,
                'sections_json' => json_encode($sections),
                'status' => 1
            );

            $paper_id = $this->pyp_model->create_paper($data);
            $this->session->set_flashdata('flash_message', 'Test paper created successfully!');
            redirect(site_url('admin/pyp_questions/' . $paper_id), 'refresh');
            ...
            $data = array(
                ...
                'shift' => html_escape($this->input->post('shift')),
                'duration_minutes' => (int)$this->input->post('duration_minutes'),
                'positive_marks' => (float)$this->input->post('positive_marks'),
                'negative_marks' => (float)$this->input->post('negative_marks'),
                'is_free' => $this->input->post('is_free') ? 1 : 0
            );
            $this->pyp_model->update_paper($param2, $data);
            $this->session->set_flashdata('flash_message', 'Test paper updated successfully!');
            redirect(site_url('admin/pyp_papers'), 'refresh');

        } elseif ($param1 == 'delete') {
            $this->pyp_model->delete_paper($param2);
            $this->session->set_flashdata('flash_message', 'Test paper and associated questions deleted!');
            redirect(site_url('admin/pyp_papers'), 'refresh');
        }

        $page_data['papers'] = $this->pyp_model->get_all_papers();
        $page_data['page_name'] = 'pyp_papers';
        $page_data['page_title'] = 'Manage Previous Year Papers & Mock Tests';
        $this->load->view('backend/index.php', $page_data);
    }

    public function pyp_paper_form($type = 'add', $id = 0)
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        $page_data['type'] = $type;
        $page_data['exams'] = $this->pyp_model->get_exams();
        if ($type == 'edit' && $id > 0) {
            $page_data['paper'] = $this->pyp_model->get_paper($id);
        }
        $page_data['page_name'] = 'pyp_paper_add';
        $page_data['page_title'] = ($type == 'add') ? 'Create Test Paper' : 'Edit Test Paper';
        $this->load->view('backend/index.php', $page_data);
    }

    public function pyp_questions($paper_id = 0, $param1 = "", $param2 = "")
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        if ($param1 == 'add') {
            $options_en = array(
                array('id' => 'opt_1', 'text' => $this->input->post('opt_1_en')),
                array('id' => 'opt_2', 'text' => $this->input->post('opt_2_en')),
                array('id' => 'opt_3', 'text' => $this->input->post('opt_3_en')),
                array('id' => 'opt_4', 'text' => $this->input->post('opt_4_en')),
            );
            $options_hi = array(
                array('id' => 'opt_1', 'text' => $this->input->post('opt_1_hi') ?: $this->input->post('opt_1_en')),
                array('id' => 'opt_2', 'text' => $this->input->post('opt_2_hi') ?: $this->input->post('opt_2_en')),
                array('id' => 'opt_3', 'text' => $this->input->post('opt_3_hi') ?: $this->input->post('opt_3_en')),
                array('id' => 'opt_4', 'text' => $this->input->post('opt_4_hi') ?: $this->input->post('opt_4_en')),
            );

            $order = $this->db->where('paper_id', $paper_id)->count_all_results('pyp_questions') + 1;
            ...
            $data = array(
                ...
                'options_hi' => json_encode($options_hi),
                'correct_options' => $this->input->post('correct_options') ?: 'opt_1',
                'solution_en' => $this->input->post('solution_en'),
                'solution_hi' => $this->input->post('solution_hi'),
                'positive_marks' => (float)$this->input->post('positive_marks') ?: 2.00,
                'negative_marks' => (float)$this->input->post('negative_marks') ?: 0.50,
            ...
            $this->pyp_model->create_question($data);
            ...
            $this->pyp_model->update_question($param2, $data);
            $this->session->set_flashdata('flash_message', 'Question updated successfully!');
            redirect(site_url('admin/pyp_questions/' . $paper_id), 'refresh');

        } elseif ($param1 == 'delete') {
            $this->pyp_model->delete_question($param2);
            $this->session->set_flashdata('flash_message', 'Question removed!');
            redirect(site_url('admin/pyp_questions/' . $paper_id), 'refresh');
        }

        $page_data['paper'] = $paper;
        $page_data['sections'] = json_decode($paper['sections_json'], true) ?: array();
        $page_data['questions'] = $this->db->where('paper_id', $paper_id)->order_by('order_index', 'ASC')->get('pyp_questions')->result_array();
        $page_data['page_name'] = 'pyp_questions';
        $page_data['page_title'] = 'Questions Bank: ' . $paper['title'];
        $this->load->view('backend/index.php', $page_data);
    }

    public function pyp_question_form($paper_id = 0, $type = 'add', $id = 0)
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        $page_data['type'] = $type;
        if ($type == 'edit' && $id > 0) {
            $page_data['question'] = $this->pyp_model->get_question($id);
        }
        $page_data['page_name'] = 'pyp_question_add';
        $page_data['page_title'] = ($type == 'add') ? 'Add New Question' : 'Edit Question';
        $this->load->view('backend/index.php', $page_data);
    }

    public function pyp_import($paper_id = 0)
    {
        if ($this->session->userdata('admin_login') != true) {
            redirect(site_url('login'), 'refresh');
        }
        ...
        if ($this->input->post('is_submitted')) {
            $import_type = $this->input->post('import_type'); // 'json_paste', 'json_file', 'csv_file'
            $parsed_questions = array();

            if ($import_type == 'json_paste') {
                $raw_json = $this->input->post('raw_json');
                $decoded = json_decode($raw_json, true);
                if (is_array($decoded)) {
                    $parsed_questions = isset($decoded['questions']) ? $decoded['questions'] : $decoded;
                }
            } elseif ($import_type == 'json_file' && isset($_FILES['file_json']['tmp_name']) && is_uploaded_file($_FILES['file_json']['tmp_name'])) {
                $content = file_get_contents($_FILES['file_json']['tmp_name']);
                $decoded = json_decode($content, true);
                if (is_array($decoded)) {
                    $parsed_questions = isset($decoded['questions']) ? $decoded['questions'] : $decoded;
                }
            } elseif ($import_type == 'csv_file' && isset($_FILES['file_csv']['tmp_name']) && is_uploaded_file($_FILES['file_csv']['tmp_name'])) {
                $handle = fopen($_FILES['file_csv']['tmp_name'], 'r');
                $header = fgetcsv($handle, 4096, ","); // Skip header
                while (($row = fgetcsv($handle, 4096, ",")) !== FALSE) {
                    if (count($row) >= 7 && !empty(trim($row[0]))) {
                        $parsed_questions[] = array(
                            'question_en' => trim($row[0]),
                            'question_hi' => isset($row[1]) ? trim($row[1]) : '',
                            'options_en' => array(
                                array('id' => 'opt_1', 'text' => trim($row[2] ?? '')),
                                array('id' => 'opt_2', 'text' => trim($row[3] ?? '')),
                                array('id' => 'opt_3', 'text' => trim($row[4] ?? '')),
                                array('id' => 'opt_4', 'text' => trim($row[5] ?? '')),
                            ),
                            'options_hi' => array(
                                array('id' => 'opt_1', 'text' => trim($row[2] ?? '')),
                                array('id' => 'opt_2', 'text' => trim($row[3] ?? '')),
                            ...
                            'correct_options' => trim($row[6] ?? 'opt_1'),
                            'solution_en' => trim($row[7] ?? ''),
                            'solution_hi' => trim($row[8] ?? ''),
                            'section_id' => trim($row[9] ?? 'sec_reasoning'),
                            'difficulty' => trim($row[10] ?? 'MEDIUM'),
                            'positive_marks' => !empty($row[11]) ? (float)$row[11] : (float)$paper['positive_marks'],
                            'negative_marks' => !empty($row[12]) ? (float)$row[12] : (float)$paper['negative_marks']
                        );
                    }
                }
                fclose($handle);
            ...
            if (!empty($parsed_questions)) {
                $count = $this->pyp_model->batch_import_questions($paper_id, $parsed_questions);
                $this->session->set_flashdata('flash_message', "Successfully imported {$count} questions into {$paper['title']}!");
                redirect(site_url('admin/pyp_questions/' . $paper_id), 'refresh');
            } else {
                $this->session->set_flashdata('error_message', 'Could not parse questions. Please check format.');
            }
        }

        $page_data['paper'] = $paper;
        $page_data['sections'] = json_decode($paper['sections_json'], true) ?: array();
        $page_data['page_name'] = 'pyp_import';
        $page_data['page_title'] = 'Bulk Import Questions: ' . $paper['title'];
        $this->load->view('backend/index.php', $page_data);
    }

    public function pyp_sample_template($format = 'csv')
    {
        if ($format == 'json') {
            $sample = array(
                array(
                    ...
                    "options_en" => array(
                        array("id" => "opt_1", "text" => "New Delhi"),
                        array("id" => "opt_2", "text" => "Mumbai"),
                        array("id" => "opt_3", "text" => "Kolkata"),
                        array("id" => "opt_4", "text" => "Chennai")
                    ),
                    "options_hi" => array(
                        array("id" => "opt_1", "text" => "नई दिल्ली"),
                        array("id" => "opt_2", "text" => "मुंबई"),
                        array("id" => "opt_3", "text" => "कोलकाता"),
                        array("id" => "opt_4", "text" => "चेन्नई")
                    ),
                    "correct_options" => "opt_1",
                    "solution_en" => "<p>New Delhi is the official capital of India.</p>",
                    "solution_hi" => "<p>नई दिल्ली भारत की आधिकारिक राजधानी है।</p>",
                    "difficulty" => "EASY",
                    "positive_marks" => 2.0,
                    "negative_marks" => 0.5
                )
            ...
            header('Content-Disposition: attachment; filename="sample_questions.json"');
            ...
            fputcsv($output, array('question_en', 'question_hi', 'opt_1', 'opt_2', 'opt_3', 'opt_4', 'correct_options', 'solution_en', 'solution_hi', 'section_id', 'difficulty', 'positive_marks', 'negative_marks'));
            fputcsv($output, array('What is 15 + 25?', '15 + 25 का मान क्या है?', '35', '40', '45', '50', 'opt_2', '<p>Simple addition: $15 + 25 = 40$</p>', '<p>सरल जोड़: $15 + 25 = 40$</p>', 'sec_quant', 'EASY', '2.0', '0.5'));
            fclose($output);
            exit;
        }
    }
