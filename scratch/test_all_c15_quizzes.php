...
echo "=== TESTING IMPROVED ROBUST PARSER ON ALL COURSE 15 QUIZZES ===\n";
while ($qz = $quizzes->fetch_assoc()) {
    $q_res = $conn->query("SELECT id, title, options, correct_answers FROM question WHERE quiz_id = {$qz['id']}");
    $cnt = 0;
    $bare = 0;
    $empty = 0;
    while ($r = $q_res->fetch_assoc()) {
        ...
        if (empty(trim(strip_tags($p['q_en'])))) $empty++;
    ...
    $grand_empty += $empty;
    $grand_bare += $bare;
    echo "Quiz #{$qz['id']}: '{$qz['title']}' ({$cnt} questions) -> Bare: {$bare}, Empty: {$empty}, OK: " . ($cnt - $bare) . "\n";
}

echo "\n--- GRAND TOTALS ---\n";
echo "Total Questions: {$grand_total}\n";
echo "Empty Questions: {$grand_empty}\n";
echo "Bare Options: {$grand_bare}\n";
echo "Success Rate: " . round((($grand_total - $grand_bare) / $grand_total) * 100, 2) . "%\n";
